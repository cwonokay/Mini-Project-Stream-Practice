...
public class Main {
    ...
    public static void printTuesdays(List<Entry> entries) {
        ...
        // write stream
        entries.stream().filter(entry -> entry.getDay() == Day.TUESDAY)
    ...
    public static void countTueWedThur(List<Entry> entries) {
        ...
        long count1 = 0;
        // write stream
        count1 = entries.stream().filter(entry -> entry.getDay() == Day.TUESDAY || entry.getDay() == Day.WEDNESDAY || entry.getDay() ==Day.THURSDAY)
                .count();
...
//         write for loop
       ...
       // write stream use collect as the terminal operation
        weekends = entries.stream().filter(entry -> entry.getDay() == Day.SATURDAY || entry.getDay() == Day.SUNDAY)
    ...
    public static void weekdaySet(List<Entry> entries) {
        ...
        // write stream
        weekdays = entries.stream().filter(entry -> entry.getDay() != Day.SATURDAY && entry.getDay() != Day.SUNDAY)
